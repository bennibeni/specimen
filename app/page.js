  ...
  return (
    ...
    >
      ...
      {Array.from({ length: TOTAL_LOCI }, (_, i) => {
        ...
        const isActive =
          activeTick !== null &&
          (i === activeTick || t4Partner(activeTick) === i);
        return (
      ...
      {messageData.map((bit, i) => {
        ...
        return (
          <div
            ...
            title={
              inCode
                ? `messageData[${i}] — incluso nel codice`
                : `messageData[${i}] — non usato nel codice`
            }
          >
            {computed ? bit : "·"}
          </div>
  ...
  return (
    <div className="pf8-wrap">
      <div
        className="pf8-grid"
        role="img"
        aria-label={`Coordinata PF8: riga ${pf8.row}, colonna ${pf8.col}`}
      >
        {cells.map(({ r, c, active }) => (
          <div
            key={`${r}-${c}`}
            className={`pf8-cell ${active ? "pf8-cell--active" : ""}`}
          />
        ))}
      </div>
      <div className="pf8-caption">
        riga <strong>{pf8.rowBits}</strong> = {pf8.row} · colonna{" "}
        <strong>{pf8.colBits}</strong> = {pf8.col}
      </div>
    </div>
  );
}
...
function PhenotypeRow({ messageData, tick }) {
  return (
    <div
      ...
      aria-label="Fenotipo: 6 caratteri decodificati dal codice"
    >
      {PHENOTYPE_TRAITS.map((trait, i) => {
        ...
        if (computed) {
          if (isChromosomal) {
            ...
            stateClass = value
              ? "trait-phenotype--dominant"
              : "trait-phenotype--recessive";
          }
        }
...
function randomGenome() {
  return Array.from({ length: 4 }, () =>
    Array.from({ length: 4 }, () =>
      Math.random() < RANDOM_ALLELE_PROBABILITY ? 1 : 0,
    ),
  );
}
  ...
  return (
    <div className="panel explainer">
      <details className="log-details">
        <summary className="panel-title log-summary">Guida al modello</summary>

        <h3 className="explainer-h3">1 — Dal genoma al fenotipo</h3>
        <p className="explainer-p">
          Ogni cellula parte da un genoma 4×4 (16 alleli). Coppie di alleli
          vengono risolte per dominanza (<code>OR</code>: basta un allele
          dominante perché il locus si esprima) in 8 loci. Gli 8 loci
          attraversano una trasformazione XOR ciclica con shift 2 (
          <code>write_t = L_t ⊕ L_(t+2 mod 8)</code>), producendo 8 bit di
          output. I primi 6 di questi bit sono il <strong>codice</strong>, che è
          anche il fenotipo: 6 caratteri binari, 2⁶ = 64 combinazioni possibili,
          decodificate come coordinata riga/colonna in una griglia 8×8 (PF8).
        </p>
          ...
          <tbody>
            {PHENOTYPE_TRAITS.map((trait) => (
              <tr key={trait.id}>
                <td>
                  {trait.label}
                  {trait.mechanism === "chromosomal" && (
                    <span className="trait-flag">†</span>
                  )}
                </td>
                <td>{trait.off}</td>
                <td>{trait.on}</td>
                <td
                  className={`explainer-mechanism ${
                    trait.mechanism === "chromosomal"
                      ? "explainer-mechanism--chromosomal"
                      : ""
                  }`}
                >
                  {trait.mechanism === "chromosomal"
                    ? "cromosomico (Y presente)"
                    : "dominanza (OR alleli)"}
                </td>
              </tr>
            ))}
        ...
        <p className="explainer-note">
          † il sesso non è un locus a dominanza mendeliana: i due bit sorgente
          rappresentano i due cromosomi sessuali (X/Y), non due alleli dello
          stesso gene in competizione. "Basta un Y per essere maschio" coincide
          numericamente con un OR, ma non c'è dominanza fra varianti — per
          questo nel pannello Fenotipo e nel genoma (celle in ciano) non viene
          mai colorato come "dominante". Capelli e pelle restano semplificazioni
          didattiche di tratti poligenici. Occhi, lattosio e vista sono gli
          esempi più fedeli a un vero locus mendeliano singolo.
        </p>

        <h3 className="explainer-h3">
        ...
        <p className="explainer-p">
          I loci L2-L5 contribuiscono a <strong>due</strong> fenotipi ciascuno
          (una volta come base del proprio tick, una volta come partner del tick
          precedente di 2 posizioni). L0, L1, L6, L7 contribuiscono a un solo
          fenotipo ciascuno — i primi due solo come base, gli ultimi due solo
          come partner, dato che i tick 6 e 7 esistono ma non producono un
          proprio carattere (il codice usa solo i primi 6 bit). I loci centrali
          hanno quindi un peso fenotipico maggiore di quelli alle estremità
          della sequenza.
        </p>

        <h3 className="explainer-h3">4 — L'anello dei loci</h3>
        <p className="explainer-p">
          Le 8 connessioni base→partner non formano un unico ciclo di lunghezza
          8, ma si scompongono in due quadrati indipendenti (0-2-4-6 e 1-3-5-7),
          perché lo shift 2 non è coprimo con 8. Per questo la trasformazione
          non è iniettiva: genomi diversi possono produrre lo stesso fenotipo, e
          con alcuni genomi metà della sequenza collassa a zero (il caso
          "silent_message" nel pannello di ispezione).
        </p>
        ...
        <p className="explainer-p">
          Con il genoma <strong>casuale</strong>, ogni allele ha probabilità{" "}
          <code>p = 1 − √0.5 ≈ 0.293</code>, non 0.5. Questo valore non è
          arbitrario: risolve esattamente l'equazione{" "}
          <code>1 − (1 − p)² = 0.5</code>, cioè rende ciascuno degli 8 loci un
          bit equiprobabile (probabilità esatta 0.5, non approssimata),
          nonostante <code>resolveDominance</code> usi un OR che normalmente
          sbilancia i loci verso 1.
        </p>
        <p className="explainer-p">
          Dato che gli 8 loci sono indipendenti fra loro (ciascuno deriva da una
          coppia di celle del genoma disgiunta dalle altre) e ciascuno è un bit
          equo, il vettore degli 8 loci è uniforme su tutti i 2⁸ = 256 pattern
          possibili. La regola <code>bit_t = L_t ⊕ L_(t+2)</code> è una
          trasformazione lineare su GF(2) — lo XOR è l'addizione di quel campo.
          Una mappa lineare suriettiva applicata a un input uniforme produce un
          output uniforme su tutta la sua immagine: ogni fenotipo riceve
          esattamente 256 / 64 = 4 dei 256 pattern di loci, nessuno escluso né
          favorito.
        </p>
        <p className="explainer-note">
          Risultato più forte di un semplice 50/50 per ciascun carattere preso
          singolarmente: qui anche tutte le{" "}
          <strong>combinazioni congiunte</strong> dei 6 caratteri sono
          equiprobabili, nonostante i bit condividano dei loci (es. L2
          contribuisce sia a Pelle che a Sesso) e siano quindi correlati come
          funzioni — la linearità della trasformazione garantisce comunque
          l'uniformità esatta del vettore congiunto, non solo dei margini.
        </p>
      </details>
    </div>
  );
...
export default function T4CellPage() {
  ...
  const [genome, setGenome] = useState(() =>
    DEFAULT_GENOME_4X4.map((row) => [...row]),
  );
    ...
    <main className="page">
      ...
      <div className="sheet">
        <div className="hero">
          ...
          <span
            className={`status-pill ${inspection.status === "stable" ? "status-pill--stable" : ""}`}
          >
        ...
        <div className="grid-two">
          <div className="panel">
            ...
            <GenomeGrid
              genome={genome}
              onToggle={handleToggleAllele}
              highlightCells={SEX_CHROMOSOME_CELLS}
            />
            ...
            <p className="genome-caption genome-caption--sex">
              le due celle evidenziate in ciano (riga 1) sono i cromosomi
              sessuali, non alleli: X o Y.
            </p>
          </div>

          <div className="panel">
            <p className="panel-title" style={{ textAlign: "center" }}>
              Anello dei loci — shift {SHIFT_K}
            </p>
            <p className="ring-formula">
              write<sub>t</sub> = L<sub>t</sub> ⊕ L
              <sub>
                (t+{SHIFT_K}) mod {TOTAL_LOCI}
              </sub>
            </p>
        ...
        <div className="panel" style={{ marginBottom: 24 }}>
          ...
          <p className="bits-caption">
            sesso · capelli · pelle → riga PF8 — occhi · lattosio · vista →
            colonna PF8 (2⁶ = 64 fenotipi)
          </p>
          <p className="bits-caption">
            † il sesso non segue una dominanza mendeliana: è determinato dal
            sistema cromosomico XY, non da un singolo locus — qui è solo
            un'etichetta binaria del modello.
          </p>
        </div>
              ...
              {cell.history.length === 0 ? (
                ...
                cell.history.map((event) => (
                  <div className="log-row" key={event.index}>
                    ...
                    <span>
                      L{event.index} ⊕ L{event.partnerIndex} ={" "}
                      {event.locusValue} ⊕ {event.partnerValue} ={" "}
          ...
          <div className="footer-grid">
            <div>
              ...
              <p className="stat-value">
                {inspection.complete ? "completa" : "incompleta"}
              </p>
            </div>
            <div>
